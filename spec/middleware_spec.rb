...
def mock_app(options = {}, conditions = {})
  ...
  builder.use WebSnap::Middleware, options, conditions
  builder.run main_app
  @app = builder.to_app
end

describe WebSnap::Middleware do
  ...
  describe "#call" do
      ...
      describe ":only" do
        ...
        describe "regex" do
          describe "one" do
            ...
            context "matching" do
              specify do
                get 'http://www.example.org/public/test.jpg'
                last_response.headers["Content-Type"].should == "image/jpeg"
                last_response.body.bytesize.should == WebSnap.new("Hello world!").to_bytes.bytesize
              end
            end

            context "not matching" do
              specify do
                get 'http://www.example.org/secret/test.jpg'
    ...
    describe "remove .jpg from PATH_INFO and REQUEST_URI" do
      before { mock_app }

      context "matching" do
        specify do
          get 'http://www.example.org/public/file.jpg'
  ...
  describe "#translate_paths" do
    before do
      @jpg = WebSnap::Middleware.new({})
      @env = { 'REQUEST_URI' => 'http://example.com/document.jpg', 'rack.url_scheme' => 'http', 'HTTP_HOST' => 'example.com' }
    end
    ...
    it "should correctly parse relative url with single quotes" do
      ...
      body = @jpg.send :translate_paths, @body, @env
  ...
  describe "#translate_paths with root_url configuration" do
    before do
      @jpg = WebSnap::Middleware.new({})
      @env = { 'REQUEST_URI' => 'http://example.com/document.jpg', 'rack.url_scheme' => 'http', 'HTTP_HOST' => 'example.com' }
      WebSnap.root_url = "http://example.net/"
    end
    ...
    after do
      WebSnap.root_url = nil
    end
  end

  it "should not get stuck rendering each request as jpg" do
    mock_app
    # false by default. No requests.
    @app.send(:rendering_jpg?).should be_false
    ...
    get 'http://www.example.org/public/file.jpg'
    @app.send(:rendering_jpg?).should be_true

    # Restore to false on any non-jpg request.
    get 'http://www.example.org/public/file'
    @app.send(:rendering_jpg?).should be_false
  end
